...
export function controlClasses(className?: string): string {
  return cx(
    'w-full rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm text-stone-800',
...
export function Select({ className, children, ...props }: React.ComponentProps<'select'>) {
  return (
    <select className={controlClasses(cx('appearance-none pr-8', className))} {...props}>
      {children}
    </select>
  );
}
